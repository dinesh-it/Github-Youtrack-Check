...
if($ENV{GITHUB_APP_KEY_FILE}) {
    ...
    # Token file will be generated by the www container
    # We try again with 1 second interval for 10 seconds
    # until we see a token file with latest values
    my $retry_count = 10;
    while($retry_count--) {
        eval {
            $gt->get_access_token;
...
sub pool_gh_pending_prs {
    ...
    print "Pooling for latest changes from github...\n";
...
sub add_pending_commits {
    ...
    print "Refreshing refreshing latest commits for $repo ...\n";
    my $gh_token = $gt ? $gt->get_access_token : $ENV{GITHUB_TOKEN};

    my $ua = LWP::UserAgent->new();
    $ua->default_header('Authorization' => "Bearer $gh_token");

    my $since_time = DateTime->from_epoch(epoch => $last_pooled_at) . 'Z';
